                         ...
                         CharacterId:(NSNumber *)characterId
{
   ...
   if (self)
   {
      ...
      self.commonName = @"Industry Jobs";
      [self.uri appendString:@"char/IndustryJobs.xml.aspx"];
      [self.uriArguments addEntriesFromDictionary:@{@"keyID":keyId,
                                                    @"vCode":vCode,
                                                    @"characterID":[characterId stringValue]}];
      self.cakAccessMask = @128;
...
-(void)configureObjectBuilders
{
   ObjectBlueprint *job = [[EVEIndustryJob new] objectBlueprint];
   [job setXmlKeypath:@"eveapi.result.rowset.row"];
   
   // Add the blueprints to our collection
   [self.objectBlueprints addObject:job];
...
-(void)requestOperationSucceededWithObjects:(NSArray *)objects
{
   ...
   {
      if ([object class] == [EVEIndustryJob class])
      {
