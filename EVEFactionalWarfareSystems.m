...
-(EVEFactionalWarfareSystems *)init
{
   ...
   if (self)
   {
      ...
      [self.uri appendString:@"map/FacWarSystems.xml.aspx"];
...
-(void)configureObjectBuilders
{
   ObjectBlueprint *solarSystem = [[EVESolarSystemFactionWarfareStatus new] objectBlueprint];
   [solarSystem setXmlKeypath:@"eveapi.result.rowset.row"];
   
   // Add the blueprints to our collection
   [self.objectBlueprints addObject:solarSystem];
...
-(void)requestOperationSucceededWithObjects:(NSArray *)objects
{
   ...
   {
      if ([object class] == [EVESolarSystemFactionWarfareStatus class])
      {
